...
were silent: they produced plausible-looking rows that were simply wrong.
"""
...
class TestSmartRecruitersPostingUrl:
    ...
    def test_the_list_endpoint_carries_no_description_and_none_is_invented(self) -> None:
        """The old code read a ``jobAd`` key the list response does not have, so every
...
class TestLeverApplyUrl:
    def test_the_apply_url_is_actually_written_so_the_fallback_works(self) -> None:
        """``get_application_url`` read ``raw_data["apply_url"]``, which nothing ever set —
        dead code that silently returned the posting URL for every Lever job."""
        src = LeverSource()
